...
func handleClient(clientConn net.Conn) error {
	...
	// レスポンスメッセージをファイルとして出力したいならコメントアウトをはずす
	// uuid := uuid.NewString()
	// filename := fmt.Sprintf("../http_messages/client_recv_from_server_%s", uuid)
	// if err := os.WriteFile(filename, []byte(responseMessage), os.ModePerm); err != nil {
